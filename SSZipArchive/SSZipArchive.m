...
int _zipOpenEntry(zipFile entry, NSString *name, const mz_zip_fileinfo *zipfi, int level, NSString *password, BOOL aes);
...
+ (BOOL)isFilePasswordProtectedAtPath:(NSString *)path {
    // Begin opening
    zipFile zip = mzunzOpen(path.fileSystemRepresentation);
    if (zip == NULL) {
        return NO;
    }
    
    BOOL passwordProtected = NO;
    int ret = mzunzGoToFirstFile(zip);
    if (ret == UNZ_OK) {
        do {
            ret = mzunzOpenCurrentFile(zip);
            if (ret != UNZ_OK) {
                // attempting with an arbitrary password to workaround `mzunzOpenCurrentFile` limitation on AES encrypted files
                ret = mzunzOpenCurrentFilePassword(zip, "");
                mzunzCloseCurrentFile(zip);
            ...
            ret = mzunzGetCurrentFileInfo(zip, &fileInfo, NULL, 0, NULL, 0, NULL, 0);
            mzunzCloseCurrentFile(zip);
            if (ret != UNZ_OK) {
                break;
    ...
    if (ret == UNZ_OK) {
        do {
            if (pw.length == 0) {
                ...
                ret = mzunzOpenCurrentFilePassword(zip, [pw cStringUsingEncoding:NSUTF8StringEncoding]);
            }
            if (ret != UNZ_OK) {
            ...
            if (ret != UNZ_OK) {
                ...
                int readBytes = mzunzReadCurrentFile(zip, buffer, (unsigned)MIN(10UL,fileInfo.uncompressed_size));
                if (readBytes < 0) {
      ...
      completionHandler:(void (^_Nullable)(NSString *path, BOOL succeeded, NSError * _Nullable error))completionHandler
{
    ...
    mzunzGetGlobalInfo(zip, &globalInfo);
    
    // Begin unzipping
    int ret = 0;
    ret = mzunzGoToFirstFile(zip);
    ...
    do {
        ...
        @autoreleasepool {
            if (password.length == 0) {
                ...
                ret = mzunzOpenCurrentFilePassword(zip, [password cStringUsingEncoding:NSUTF8StringEncoding]);
            }
            ...
            mzunzGetCurrentFileInfo(zip, &fileInfo, filename, fileInfo.size_filename + 1, NULL, 0, NULL, 0);
            ...
            if (isDirectory && !fileIsSymbolicLink) {
                ...
                int readBytes = mzunzReadCurrentFile(zip, buffer, 4096);
                if (readBytes >= 0) {
                    ...
                    while (fp) {
                        ...
                        readBytes = mzunzReadCurrentFile(zip, buffer, 4096);
                        if (readBytes < 0) {
                            // Let's assume error Z_DATA_ERROR is caused by an invalid password
                            // Let's assume other errors are caused by Content Not Readable
                            success = NO;
            ...
            else
            {
                ...
                int bytesRead = 0;
                while ((bytesRead = mzunzReadCurrentFile(zip, buffer, 4096)) > 0)
                {
                    buffer[bytesRead] = 0;
            ...
            crc_ret = mzunzCloseCurrentFile(zip);
...
- (BOOL)writeSymlinkFileAtPath:(NSString *)path withFileName:(nullable NSString *)fileName compressionLevel:(int)compressionLevel password:(nullable NSString *)password AES:(BOOL)aes
{
    ...
    int error = mzZipOpenNewFileInZip5(_zip, fileName.fileSystemRepresentation, &zipInfo, NULL, 0, NULL, 0, NULL, Z_DEFLATED, compressionLevel, 0, -MAX_WBITS, DEF_MEM_LEVEL, Z_DEFAULT_STRATEGY, password.UTF8String, aes, version_madeby, 0, 0);
    mzZipWriteInFileInZip(_zip, link_path, (uint32_t)strlen(link_path));
    mzZipCloseFileInZip(_zip);
    return error == ZIP_OK;
}
...
- (BOOL)open
{
    ...
    _zip = mzZipOpen(_path.fileSystemRepresentation, APPEND_STATUS_CREATE);
    return (NULL != _zip);
}
...
{
    NSAssert((_zip == NULL), @"Attempting to open an archive which is already open");
    _zip = mzZipOpen(_path.fileSystemRepresentation, APPEND_STATUS_ADDINZIP);
    return (NULL != _zip);
}
...
- (BOOL)writeFolderAtPath:(NSString *)path withFolderName:(NSString *)folderName withPassword:(nullable NSString *)password
{
    ...
    mzZipWriteInFileInZip(_zip, buffer, 0);
    mzZipCloseFileInZip(_zip);
    return error == ZIP_OK;
}
...
- (BOOL)writeFileAtPath:(NSString *)path withFileName:(nullable NSString *)fileName compressionLevel:(int)compressionLevel password:(nullable NSString *)password AES:(BOOL)aes
{
    ...
    while (!feof(input) && !ferror(input))
    {
        ...
        mzZipWriteInFileInZip(_zip, buffer, len);
    }
...
- (BOOL)writeData:(NSData *)data filename:(nullable NSString *)filename compressionLevel:(int)compressionLevel password:(nullable NSString *)password AES:(BOOL)aes
{
    ...
    mzZipWriteInFileInZip(_zip, data.bytes, (unsigned int)data.length);
...
- (BOOL)close
{
    ...
    int error = mzZipClose(_zip, NULL);
    _zip = nil;
    return error == ZIP_OK;
}
...
+ (void)zipInfo:(mz_zip_fileinfo *)zipInfo setAttributesOfItemAtPath:(NSString *)path
{
...
+ (void)zipInfo:(mz_zip_fileinfo *)zipInfo setDate:(NSDate *)date
{
...
int _zipOpenEntry(zipFile entry, NSString *name, const mz_zip_fileinfo *zipfi, int level, NSString *password, BOOL aes)
{
    ...
    return mzZipOpenNewFileInZip5(entry, name.fileSystemRepresentation, zipfi, NULL, 0, NULL, 0, NULL, Z_DEFLATED, level, 0, -MAX_WBITS, DEF_MEM_LEVEL, Z_DEFAULT_STRATEGY, password.UTF8String, aes, made_on_darwin, flag_base, 1);
}
